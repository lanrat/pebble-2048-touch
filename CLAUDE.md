...
**VNC keymap workaround:** On some setups the current SDK's qemu fails with `qemu-pebble: -vnc :1: could not read keymap file: 'en-us'` because `pc-bios/keymaps` is missing from the active SDK's toolchain. In that case, run the emulator commands **without** `--vnc` — on this machine the default backend works headlessly. If `--vnc` is required, symlink the keymaps from an older SDK that has them, e.g.:

```bash
ln -s ~/.pebble-sdk/SDKs/4.9.148/toolchain/lib/pc-bios \
      ~/.pebble-sdk/SDKs/current/toolchain/lib/pc-bios
```

**Install hangs / "App install failed":** If `pebble install --emulator <platform>` hangs indefinitely (the firmware boots and pypkjs connects, but the install times out partway through `PutBytes`), the emulator's flash state is likely corrupted. Wipe it and retry:

```bash
pebble kill
rm -rf ~/.pebble-sdk/<sdk-version>/<platform>/qemu_spi_flash.bin \
       ~/.pebble-sdk/<sdk-version>/<platform>/app_cache
pebble install --emulator <platform>
```

For example, for emery on SDK 4.9.169: remove `~/.pebble-sdk/4.9.169/emery/qemu_spi_flash.bin` and `~/.pebble-sdk/4.9.169/emery/app_cache`.
...
This project is configured as a watchapp (`"watchface": false` in package.json).
